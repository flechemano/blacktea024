...
async function precompile0f(opts) {
    ...
    if (opts._debug) {
        opts._debug(`Run BLS12G2MULTIEXP (0x0f) precompile data=${(0, util_1.short)(opts.data)} length=${opts.data.length} gasLimit=${opts.gasLimit} gasUsed=${gasUsed}`);
    }
    ...
    if (opts._debug) {
        opts._debug(`BLS12G2MULTIEXP (0x0f) return value=${returnValue.toString('hex')}`);
    }
    return {
